...
export interface ReviewFilterDrawerProps {
  ...
  onApply: (filters: ReviewFilterState) => void;
  onReset: () => void;
}
...
export default function ReviewFilterDrawer({
  ...
  const [localFilters, setLocalFilters] = useState<ReviewFilterState>(filters);
        ...
        <StyledDialogContent dividers sx={{ px: 2.5, py: 2 }}>
          <StyledSimpleBar>
            <Stack spacing={2}>
              ...
              <Box>
                <FilterText>{t("review.rating")}</FilterText>
                <ToggleButtonGroup
                  value={localFilters.rating ?? null}
                  exclusive
