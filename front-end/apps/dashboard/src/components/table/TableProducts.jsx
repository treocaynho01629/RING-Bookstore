...
import { currencyFormat, getBookType, idFormatter, useDeepEffect } from "@ring/shared";
    ...
    <Stack width="100%" spacing={1} my={2} direction={{ xs: "column", md: "row" }}>
      ...
      >
        {Object.values(BookType).map((type, index) => {
          const typeMeta = getBookType(type);
          return (
            <MenuItem key={`type-${typeMeta.value}-${index}`} value={typeMeta.value}>
              <Checkbox sx={{ py: 0.5, pr: 1, pl: 0 }} disableRipple checked={types?.includes(typeMeta.value)} />
              <ListItemText primary={typeMeta.label} />
            </MenuItem>
          );
        })}
      </TextField>
        ...
        return (
          <TableRow hover aria-checked={isItemSelected} tabIndex={-1} key={id}>
            ...
            <TableCell component="th" id={labelId} scope="row" padding="none" align="center">
              <Link href={`/product/${id}`}>{idFormatter(id)}</Link>
            </TableCell>
            <TableCell align="left">
              <Link href={`/product/${id}`} style={{ display: "flex", alignItems: "center" }}>
                <LazyLoadImage
